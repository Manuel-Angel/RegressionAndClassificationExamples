 ...
 */
public class NewLagMaker extends TSLagMaker implements SupervisedFilter,
Serializable {
...
protected TSLagMaker.PeriodicityHandler m_dateBasedPeriodicity =
  new TSLagMaker.PeriodicityHandler();
protected TSLagMaker.Periodicity m_userHintPeriodicity = null; // Periodicity.UNKNOWN;
 ...
 */
public static TSLagMaker.PeriodicityHandler determinePeriodicity(Instances insts,
  String timeName, TSLagMaker.Periodicity userHint) {
  ...
  TSLagMaker.PeriodicityHandler result = new TSLagMaker.PeriodicityHandler();

  if (timeIndex < 0) {
    result.setPeriodicity(TSLagMaker.Periodicity.UNKNOWN);
    result.setDeltaTime(Utils.missingValue());
    return result;
  }

  if (userHint != null && userHint != TSLagMaker.Periodicity.UNKNOWN
  ...
  if (insts.attribute(timeIndex).isDate()) {
    ...
    if (Math.abs(oneHour - averageDelta) <= fiveMins) {
      result.setPeriodicity(TSLagMaker.Periodicity.HOURLY);
      result.setDeltaTime(oneHour);
      result.setDateTimeStampInitial(initialTS);
      result.setDateTimeStampFinal(finalTS);
      return result;
    ...
    if (Math.abs(oneDay - averageDelta) <= oneHour) {
      result.setPeriodicity(TSLagMaker.Periodicity.DAILY);
      result.setDeltaTime(oneDay);
      result.setDateTimeStampInitial(initialTS);
      result.setDateTimeStampFinal(finalTS);
      return result;
    ...
    if (Math.abs(oneWeek - averageDelta) <= (oneDay / 4.0)) {
      result.setPeriodicity(TSLagMaker.Periodicity.WEEKLY);
      result.setDeltaTime(oneWeek);
      result.setDateTimeStampInitial(initialTS);
      result.setDateTimeStampFinal(finalTS);
      return result;
    ...
    if (Math.abs(thirtyDays - averageDelta) <= (oneDay * 3.0)) {
      result.setPeriodicity(TSLagMaker.Periodicity.MONTHLY);
    ...
    if (Math.abs(approxQuarter - averageDelta) <= oneWeek) {
      result.setPeriodicity(TSLagMaker.Periodicity.QUARTERLY);
    ...
    if (Math.abs(oneYear - averageDelta) <= (oneDay * 2.0)) {
      result.setPeriodicity(TSLagMaker.Periodicity.YEARLY);
      result.setDeltaTime(oneYear);
      result.setDateTimeStampInitial(initialTS);
      result.setDateTimeStampFinal(finalTS);
      return result;
...
public TSLagMaker.Periodicity getPeriodicity() {
  if (!m_adjustForTrends || m_useArtificialTimeIndex) {
    // return null;
    return TSLagMaker.Periodicity.UNKNOWN;
  }

  return m_userHintPeriodicity != null
    && m_userHintPeriodicity != TSLagMaker.Periodicity.UNKNOWN ? m_userHintPeriodicity
...
public
  void setPeriodicity(TSLagMaker.Periodicity toUse) {
    ...
    if (result.attribute(m_timeStampName).isDate()) {
      ...
      for (int i = 0; i < result.numInstances(); i++) {
        if (!result.instance(i).isMissing(origIndex)) {
          if (m_dateBasedPeriodicity.getPeriodicity() == TSLagMaker.Periodicity.MONTHLY
            || m_dateBasedPeriodicity.getPeriodicity() == TSLagMaker.Periodicity.WEEKLY
            || m_dateBasedPeriodicity.getPeriodicity() == TSLagMaker.Periodicity.QUARTERLY) {
